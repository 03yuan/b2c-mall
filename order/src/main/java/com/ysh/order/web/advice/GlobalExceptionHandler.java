package com.ysh.order.web.advice;
...
/**
 * order 服务全局异常处理：
 * 把参数校验/业务校验/状态机非法流转异常转成带中文原因的 JSON，避免笼统的 500。
 * - 参数错误、业务规则不满足、非法状态流转 -> 400
 * - 订单不存在（消息含“不存在”）-> 404
 * - 其余未预期异常 -> 500
 */
@Slf4j
...
public class GlobalExceptionHandler {
    ...
    /** 状态机非法流转（如重复支付、未支付就发货、已完成再操作） */
    @ExceptionHandler(IllegalStateException.class)
    public ResponseEntity<BaseResponseVO<Void>> handleIllegalState(IllegalStateException ex) {
        return build(HttpStatus.BAD_REQUEST, ex.getMessage());
    }
